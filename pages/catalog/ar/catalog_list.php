                      ...
                      <ul class="<? if($ex[1] == $cat['ID']){ echo false;}else{ echo 'sub-menu';}?> custom-border">
                        <li>
                          <a href="<?=mdir("product/$innerPageName/$subc[SID]") ?>">
                            <span class="mm-text"><?=$subc['NAME_AR']?></span>
                          </a>
                        </li>
                      </ul>
            ...
            <div class="widgets-area mb-9 pd-top">
              <h2 class="widgets-title mb-5">Certificates</h2>
                ...
                <ul class="widgets-category">
                  <? //foreach($brand_list as $brand):?>
                    <li>
                      <a href="https://khoshbakhtgroup.com/ar/quality-control">رقابة جودة</a>
                    </li>
                  <? //endforeach ?>
                </ul>
              </div>
            ...
            <? foreach($cat_files as $v): ?>
              ...
              <a class="underline-link" href="<?=$get_file['file_1'];?>" download>
                ...
                <div class="middle-s download-topbar">
                    <div class="second-button">تحميل الكتالوج</div>
                </div>
              </a> 
              <? endforeach; ?> 
              </div>       
          ...
          <div class="product-topbar">
            <ul>
              <li class="page-count margin-count">
                <span><?=$count['TOTAL'];?></span> تم العثور على المنتج
              </li>
              <? foreach($cat_files as $v):?>
              <? $get_file = json_decode($v['URL'], true); ?>
              <li class="page-count margin-count margin-right" align="center">
                 <a class="underline-link" href="<?=$get_file['file_1']?>" download><i class="fa fa-download" aria-hidden="true"></i> تحميل الكتالوج</a>
              </li>
              <? endforeach;?>
            </ul>
          </div>
                  ...
                  <div class="col-xl-4 col-sm-6 pt-6">
                    <div class="product-item">
                      ...
                      <div class="product-content">
                        <a class="bg-primary p-1 text-light" href="<?=mdir("product/$innerPageName/$v[ID]")?>"><?=$v['NAME_AR']; ?></a>
                      </div>
                    </div>
                  </div>
                <? endforeach; ?>

        ...
        <div class="col-xl-3 col-lg-4 order-lg-1 order-1 pt-10 pt-lg-0">
          <div class="sidebar-area style-2">
                      ...
                      <ul class="<? if($ex[1] == $cat['ID']){ echo false;}else{ echo 'sub-menu';}?> custom-border">
                        <li>
                          <a href="<?=mdir("product/$innerPageName/$subc[SID]") ?>">
                            <span class="mm-text"><?=$subc['NAME_FR']?></span>
                          </a>
                        </li>
                      </ul>
            ...
            <div class="widgets-area mb-9 pd-top">
              <h2 class="widgets-title mb-5">Certificats</h2>
                <div class="widgets-item">
                <ul class="widgets-category">
                  <? //foreach($brand_list as $brand):?>
                    <li>
                      <a href="https://khoshbakhtgroup.com/fr/quality-control">Contrôle de qualité</a>
                    </li>
                  <? //endforeach ?>
                </ul>
              </div>
            ...
            <? foreach($cat_files as $v): ?>
              ...
              <a class="underline-link" href="<?=$get_file['file_1'];?>" download>
                ...
                <div class="middle-s download-topbar">
                    <div class="second-button">Télécharger le catalogue</div>
                </div>
              </a> 
              <? endforeach; ?> 
              </div>       
          ...
          <div class="product-topbar">
            <ul>
              <li class="page-count margin-count">
                <span><?=$count['TOTAL'];?></span> Produit trouvé
              </li>
              <? foreach($cat_files as $v): ?>
              <? $get_file = json_decode($v['URL'], true); ?>
              <li class="page-count margin-count margin-right" align="center">
                 <a class="underline-link" href="<?=$get_file['file_1'];?>" download><i class="fa fa-download" aria-hidden="true"></i> Télécharger le catalogue</a>
              </li>
              <? endforeach; ?>
            </ul>
          </div>
                  ...
                  <div class="col-xl-4 col-sm-6 pt-6">
                    <div class="product-item">
                      ...
                      <div class="product-content">
                        <a class="bg-primary p-1 text-light" href="<?=mdir("product/$innerPageName/$v[ID]")?>"><?=$v['NAME_FR']; ?></a>
                      </div>
                    </div>
                  </div>
                <? endforeach; ?>
